...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    button:hover { background: #2f5cea; }
...
function consentBody(params, error) {
  const host = (() => {
    try {
  ...
  return `<h1>포트폴리오 읽기 권한 요청</h1>
    <p>Claude 가 Signal15 의 보유 종목·자산 추이·매매 내역을 <strong>읽기 전용</strong>으로 조회하려 합니다. 본인 계정으로 로그인해 승인해 주세요.</p>
...
async function verifyOwner(email, password) {
  ...
  if (!url || !anon) return { error: '서버에 Supabase 설정이 없습니다' }
  ...
  if (data.user.id !== ownerId) return { error: '이 커넥터를 사용할 수 있는 계정이 아닙니다' }
...
export default async function handler(req, res) {
  ...
  if (!isPost) {
    sendHtml(res, 200, page('커넥터 승인', consentBody(params, '')))
    return
  }
  ...
  if (!email || !password) {
    sendHtml(
      res,
      400,
      page('커넥터 승인', consentBody(params, '이메일과 비밀번호를 입력해 주세요')),
    )
    return
  }
  ...
  if (!owner.userId) {
    sendHtml(
      res,
      401,
      page('커넥터 승인', consentBody(params, owner.error ?? '로그인에 실패했습니다')),
    )
    return
  }
