...
type Props = {
  groupName: string
  /** 지정 시 헤더 제목 전체 교체 (예: 포트폴리오 전체 진단) */
  title?: string
  loading: boolean
...
export function GroupDiagnosisModal({ groupName, title, loading, analysis, onClose }: Props) {
  const heading = title ?? `${groupName} 그룹 진단`
  return (
    <div
    ...
    >
      <div
        className="max-h-[85vh] w-full max-w-lg overflow-y-auto rounded-2xl bg-white md:max-w-4xl"
