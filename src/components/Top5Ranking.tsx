      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
        {top5.map((stock, idx) => {
          return (
            <article
              key={stock.code}
              className="rounded-2xl border border-default bg-neutral-bg p-4"
            >
              <div className="rounded-xl bg-card p-3">
                <p className="text-xs font-semibold text-amber-700">TOP{idx + 1}</p>
                <h4 className="mt-1 text-lg font-bold text-primary">{stock.name}</h4>
                <p className="mt-1 text-xs text-secondary">{stock.sector}</p>
              </div>

              <div className="mt-2 space-y-1 rounded-xl bg-card p-3 text-xs text-secondary">
                ...
                <p className="inline-flex w-full items-center gap-1 whitespace-nowrap">
                  <CircleGauge className="size-3.5 text-secondary" />
